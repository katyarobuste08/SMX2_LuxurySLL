...
Explicación:
El servicio FTP permite la transferencia de archivos entre clientes y el servidor a través de la red. En TrueNAS se implementa mediante el servidor ProFTPD. Para su funcionamiento es necesario habilitar el servicio desde la interfaz de TrueNAS, configurar los permisos de acceso a los datasets y definir los usuarios que podrán conectarse. Este servicio utiliza el puerto 21 y puertos pasivos adicionales para la transferencia de datos. Debido a que el protocolo FTP no cifra la comunicación por defecto, se recomienda utilizarlo únicamente en redes internas o mediante versiones seguras como FTPS.
...
Configurar correctamente los permisos de usuarios y grupos sobre los datasets.

Verificar que los puertos necesarios estén accesibles dentro de la red.

Aplicar medidas de seguridad adecuadas, especialmente en servicios que no cifran la comunicación.

Estos servicios permiten ofrecer diferentes métodos de acceso al almacenamiento del NAS dependiendo del sistema cliente y del nivel de seguridad requerido.
